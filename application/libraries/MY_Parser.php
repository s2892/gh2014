...
class MY_Parser extends CI_Parser {
    ...
    public function pahref($url, $content, $title='link', $target='_self',  $attributes = array())
    {
        ...
        $return = '<a href="'.base_url($url).'" type="'.$attributes['type'].'" target="'.$target.'"  title="'.$title.'" alt="'.$attributes['alt'].'">
        	'.$content.'
		</a>
        ';
        return $return;
    ...
    public function pcss($file, $path='/', $t='',$media='screen', $attributes = array())
    {
        ...
        $attributes = array_merge($defaults, $attributes);
        if($path!='/') $path = '/'.$path.'/';
        if($t=='') $t= $this->get_theme();

        $return = '<link rel="'.$attributes['rel'].'" type="'.$attributes['type'].'" href="'.base_url(config_item('theme_path').$t.$path.$file).'" media="'.$attributes['media'].'">';
        return $return;
    }
    ...
    public function pjs($file, $path='/', $t='', $attributes = array())
    {
        $defaults = array(
            'type'  => 'text/javascript'
        );
        ...
        if($t=='') $t= $this->get_theme();
        $attributes = array_merge($defaults, $attributes);
        $return = '<script type="'.$attributes['type'].'" src="'.base_url(config_item('theme_path').$t.$path.$file).'"></script>';
    ...
    public function pimg($file, $path='/', $t='', $attributes = array())
    {
        $defaults = array(
            'alt'    => '',
            'title'  => ''
        ...
        if($path!='/') $path = '/'.$path.'/';
        if($t=='') $t= $this->get_theme();
        $return = '<img src ="'.base_url(config_item('theme_path').$t.$path.$file).'" alt="'.$attributes['alt'].'" title="'.$attributes['title'].'" />';
